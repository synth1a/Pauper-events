...
def generate_html(events: list[dict], fmt_filter: str, generated_at: str,
    ...
    for date_str in sorted(by_date.keys()):
        ...
        for ev in by_date[date_str]:
            ...
            cards_html += f"""<div class="event-card" data-shop="{shop_attr}">
...
{gcal_section}<div class="filter-bar">
  ...
  <select id="shop-filter" onchange="filterByShop(this.value)">
    {shop_options}
  </select>
...
<footer>Generated by hareruya-events | データ元: <a href="https://www.hareruyamtg.com/ja/events" target="_blank" rel="noopener noreferrer">晴れる屋</a></footer>
<script>
function filterByShop(shop) {{
  ...
  cards.forEach(function(card) {{
    if (shop === 'all' || card.getAttribute('data-shop') === shop) {{
